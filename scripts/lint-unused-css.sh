...
for css in "$STYLES_DIR"/*.css; do
  ...
  for cls in $classes; do
    # Word-boundary match (hyphen-aware), exclude all stylesheets and design-system.md
    usage=$(grep -rlE "(^|[^a-zA-Z0-9_-])${cls}([^a-zA-Z0-9_-]|$)" "$REPO_ROOT/src/" 2>/dev/null \
            | grep -vE '/styles/[^/]+\.css$|design-system\.md$|/styles/README\.md$' || true)
    if [ -z "$usage" ]; then
      report+=("  ${fname}  .${cls}")
...
printf '%s\n' "${report[@]}"
echo ""
echo "Fix: delete the rule, or register usage in design-system.md §6.3"
exit 1
