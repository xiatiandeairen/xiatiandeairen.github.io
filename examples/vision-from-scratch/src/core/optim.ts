...
//   residual converges faster than plain), not absolute step counts.
...
export class SGD implements Optimizer {
  ...
  private velocity: Float64Array[]; // per-param momentum buffer
  ...
  /** v = momentum*v + grad(+wd*param); param -= lr*v. Weight decay is the coupled (L2)
   *  form, folded into the gradient. */
  step(): void {
...
/**
 * Adam / AdamW.
 * WHY bias correction: m and v start at 0, so early estimates are biased toward 0; dividing
 *   by (1 - beta^t) un-biases them. Skipping it makes the first ~1/(1-beta) steps take tiny
 *   effective steps — a real, observable slow start.
 * WHY a `decoupled` flag (AdamW vs Adam): coupled weight decay folds wd into the gradient,
 *   so the adaptive denominator ALSO scales the decay. AdamW applies decay directly to
 *   params, independent of the adaptive scaling.
 */
export class Adam implements Optimizer {
  ...
  constructor(
    params: Tensor[],
    opts: { lr: number; beta1?: number; beta2?: number; eps?: number; weightDecay?: number; decoupled?: boolean },
  ) {
    this.params = params;
    this.lr = opts.lr;
...
/** AdamW is just Adam with decoupled=true. Named for discoverability. */
export class AdamW extends Adam {
  constructor(params: Tensor[], opts: { lr: number; beta1?: number; beta2?: number; eps?: number; weightDecay?: number }) {
 ...
 * WHY global (not per-param): preserves the DIRECTION of the overall gradient while capping
 *   its magnitude. Returns the PRE-clip norm so the loop can log it (spikes = early warning
 *   of instability). FAILURE MODE without clipping: one bad batch -> huge grad -> step
 *   overshoots -> loss NaN.
 */
...
/**
 * Cosine LR schedule with linear warmup. Pure function of step => fully reproducible.
